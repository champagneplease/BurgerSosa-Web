...
export class OrdersService {
  ...
  async createOrder(data: any) {
    ...
    for (const item of data.items) {
      if (item.quantity > 6) {
        throw new BadRequestException(`No se permiten más de 6 unidades por producto`);
      }
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const modId of item.modifiers) {
          ...
          if (currentCount > 6) {
            throw new BadRequestException(`No se permiten más de 6 extras del mismo tipo`);
          }
