...
function LandingPage() {
  ...
  return (
    <div className="client-app min-h-screen bg-brand-dark text-brand-light font-display selection:bg-brand-accent selection:text-brand-dark">
      {/* Header */}
      <header className="fixed top-0 w-full z-40 bg-black/80 backdrop-blur-md border-b border-white/10 transition-all">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button 
              className="md:hidden text-brand-light/80 hover:text-brand-light"
            ...
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
              ...
              <h1 className="text-xl font-black tracking-tighter uppercase text-brand-light hidden sm:block">Burger<span className="text-brand-accent">Sosa</span></h1>
            </div>
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-6 font-medium text-sm text-brand-light/80">
            <button onClick={() => scrollToSection('catalog')} className="hover:text-brand-accent transition-colors">Catálogo</button>
            <button onClick={() => scrollToSection('about')} className="hover:text-brand-accent transition-colors">Sobre Nosotros</button>
          </nav>
          
          <button 
            onClick={() => isStoreOpen && setIsCartOpen(true)}
            className={`relative p-2 rounded-full transition-colors flex items-center gap-2 ${isStoreOpen ? 'hover:bg-brand-btn/5 cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
          >
            <ShoppingBag size={24} className="text-brand-light/80" />
            {cartCount > 0 && isStoreOpen && (
              <span className="absolute -top-1 -right-1 bg-brand-accent text-brand-dark text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full animate-in zoom-in">
                {cartCount}
              </span>
            )}
          </button>
      ...
      <main className="pt-16 max-w-5xl mx-auto relative min-h-[90vh]">
        <div className="absolute top-20 left-10 w-64 h-64 bg-brand-accent rounded-full mix-blend-screen filter blur-[100px] opacity-30 animate-pulse"></div>
        ...
        <div className="relative w-full h-[50vh] md:h-[60vh] overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-tr from-brand-dark via-brand-dark/80 to-transparent z-10" />
          <img 
            src="/banner.png" 
          ...
          <div className="absolute z-20 bottom-0 left-0 p-6 md:p-12 w-full max-w-2xl">
            {isStoreOpen ? (
              <>
                 <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-accent/10 border border-brand-accent/20 text-brand-accent text-sm font-semibold mb-4 md:mb-6 backdrop-blur-md">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-accent/90 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-accent"></span>
                    </span>
                    La parrilla está encendida
                 </div>
                 <h2 className="text-4xl md:text-7xl font-black mb-4 md:mb-6 leading-[1.1] text-brand-light drop-shadow-2xl tracking-tight">
                   La mejor hamburguesa del <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-accent/90 to-orange-500">Valle</span>.
                 </h2>
                 <p className="text-brand-light/80 text-base md:text-xl mb-6 md:mb-8 font-light max-w-lg">
                   Pedí ahora y descubrí por qué todos hablan de nuestro verdadero sabor artesanal.
                 </p>
                 <button 
                   onClick={() => scrollToSection('catalog')}
                   className="group bg-gradient-to-r from-brand-accent to-orange-500 hover:from-brand-accent/90 hover:to-orange-400 text-brand-dark px-6 md:px-8 py-3 md:py-4 rounded-full font-black text-base md:text-lg transition-all shadow-[0_0_30px_rgba(245,158,11,0.3)] hover:shadow-[0_0_50px_rgba(245,158,11,0.5)] hover:-translate-y-1 flex items-center gap-2"
                 >
                   Ver Menú Digital 
              ...
              <>
                 ...
                 <h2 className="text-4xl md:text-7xl font-black mb-4 md:mb-6 leading-[1.1] text-brand-light drop-shadow-2xl tracking-tight">
                   Nos fuimos a <span className="text-brand-light/40">descansar</span>.
                 </h2>
                 <p className="text-brand-accent text-lg md:text-xl mb-6 font-medium max-w-lg bg-brand-accent/10 p-4 rounded-xl border border-brand-accent/20">
                   {closedMessage}
                 </p>
                 <button 
                   onClick={() => window.open('https://www.instagram.com/burgersosa_/', '_blank')}
                   className="group bg-brand-btn/5 hover:bg-zinc-700 text-brand-light px-6 md:px-8 py-3 md:py-4 rounded-full font-black text-base md:text-lg transition-all flex items-center gap-2"
                 >
      ...
      {/* Footer */}
      <footer id="about" className="bg-brand-dark border-t border-zinc-900 pt-16 pb-8">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-brand-light/60">
              ...
              <li>
                <a href="https://www.instagram.com/burgersosa_/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-brand-accent hover:text-brand-accent/90 mt-4 transition-colors">
                  @burgersosa_
                </a>
              </li>
            </ul>
          ...
          <div>
            <h3 className="font-bold text-lg mb-4 text-brand-light">Legal & Contacto</h3>
        ...
        <div className="max-w-5xl mx-auto px-4 pt-8 border-t border-zinc-900 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-brand-light/40">
          ...
          <p className="flex items-center gap-1">
            ...
            <a href="https://www.instagram.com/_nachomirnd/" target="_blank" rel="noopener noreferrer" className="text-brand-accent hover:text-brand-accent/90 transition-colors font-medium">
              Nacho Miranda
            </a>
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-brand-surface h-full flex flex-col border-l border-white/10 animate-in slide-in-from-right duration-300">
            <div className="p-6 flex items-center justify-between border-b border-white/10">
               ...
               <button onClick={() => setIsCartOpen(false)} className="p-2 hover:bg-brand-btn/5 rounded-full transition-colors">
                 <X size={20} />
               </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
              {cartItems.length === 0 ? (
                ...
                cartItems.map((item) => (
                  <div key={item.id} className="bg-brand-btn/5/50 rounded-2xl p-4 flex gap-4 relative group border border-transparent hover:border-white/20 transition-colors">
                    <button 
                      onClick={() => removeCartItem(item.id)}
                      className="absolute -top-2 -right-2 bg-red-500 text-brand-light p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      <X size={14} />
                    </button>
                    <div className="w-16 h-16 bg-brand-btn/5 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                         ...
                         <span className="text-sm bg-brand-btn/5 px-2 py-1 rounded-md text-brand-light/80">x{item.quantity}</span>
                      </div>
                    </div>
                  </div>
                ))
            ...
            {cartItems.length > 0 && (
              <div className="p-6 bg-brand-surface border-t border-white/10">
                <div className="flex items-center justify-between mb-4 text-lg">
                  ...
                  <span className="text-brand-light font-bold">${cartTotal.toLocaleString('es-AR')}</span>
                </div>
                <button 
                  onClick={() => {
                    setIsCartOpen(false);
                  ...
                  className="w-full bg-brand-accent hover:bg-brand-accent/90 text-brand-dark font-bold py-4 rounded-2xl transition-transform active:scale-95 flex items-center justify-center gap-2"
                >
