  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-brand-surface rounded-3xl w-full max-w-md overflow-hidden border border-white/10 shadow-2xl relative flex flex-col max-h-[90vh]">
        ...
        <div className="w-full h-48 bg-gradient-to-tr from-brand-accent to-orange-400 relative">
        ...
        <div className="p-6 overflow-y-auto flex-1">
                     ...
                     >
                       <div className="flex items-center gap-3">
                         <div className={`w-5 h-5 rounded flex items-center justify-center border ${isSelected ? 'bg-brand-accent border-brand-accent' : 'border-zinc-600'}`}>
                           {isSelected && <div className="w-2.5 h-2.5 bg-brand-btn rounded-sm" />}
                         </div>
                         <div>
                       ...
                       {isSelected && price > 0 && (
                         <div className="flex items-center gap-3 bg-brand-surface rounded-lg p-1 border border-white/10" onClick={(e) => e.stopPropagation()}>
                           <button onClick={(e) => { e.stopPropagation(); decrementModifier(mod.id); }} className="p-1 rounded-md hover:bg-brand-btn/5 text-brand-light/60 transition-colors">
                             <Minus size={14} />
                           </button>
                           <span className="text-brand-light font-medium w-3 text-center text-sm">{count}</span>
                           <button onClick={(e) => { e.stopPropagation(); incrementModifier(mod.id); }} className="p-1 rounded-md hover:bg-brand-btn/5 text-brand-light/60 transition-colors">
                             <Plus size={14} />
                           </button>
                         </div>
                       )}
          ...
          <div className="mb-6">
            ...
            <textarea 
              ...
              className="w-full bg-brand-dark border border-white/10 text-brand-light rounded-xl p-3 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent resize-none h-20 text-sm"
            />
          </div>

          <div className="flex items-center justify-between py-4 border-t border-white/10">
            <span className="text-brand-light font-medium">Cantidad</span>
            <div className="flex items-center gap-4 bg-brand-btn/5 rounded-full p-1">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2 rounded-full hover:bg-zinc-700 text-brand-light transition-colors"><Minus size={16} /></button>
              <span className="text-brand-light font-semibold w-4 text-center">{quantity}</span>
              <button onClick={() => setQuantity(quantity < 6 ? quantity + 1 : 6)} className="p-2 rounded-full hover:bg-zinc-700 text-brand-light transition-colors"><Plus size={16} /></button>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-brand-surface border-t border-white/10">
           <button 
             ...
             className="w-full bg-brand-accent hover:bg-brand-accent/90 text-brand-dark font-bold py-4 rounded-2xl transition-transform active:scale-95 flex items-center justify-between px-6"
           >
