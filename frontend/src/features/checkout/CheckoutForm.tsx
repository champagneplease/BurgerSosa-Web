    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-brand-surface rounded-3xl w-full max-w-md overflow-hidden border border-white/10 shadow-2xl relative flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
           ...
           <button onClick={onClose} className="p-2 hover:bg-brand-btn/5 rounded-full transition-colors text-brand-light/60">
             <X size={20} />
           </button>
        </div>
          ...
          <form id="checkout-form" onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-brand-btn/5 border border-white/20 text-brand-light rounded-xl px-4 py-3 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent transition-all"
            ...
            {type === 'DELIVERY' && (
              <div className="pt-2 animate-in fade-in slide-in-from-top-2 space-y-3">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-brand-btn/5/80 hover:bg-zinc-700/80 text-brand-accent border border-white/20 hover:border-brand-accent/50 py-3 px-4 rounded-xl transition-all font-medium text-sm"
                >
                  {locationLoading ? (
                ...
                <textarea 
                  ...
                  className="w-full bg-brand-btn/5 border border-white/20 text-brand-light rounded-xl px-4 py-3 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent transition-all resize-none"
            ...
            {type === 'PICKUP' && (
              <div className="pt-2 animate-in fade-in slide-in-from-top-2 space-y-3">
                <div className="p-4 bg-brand-btn/5/80 border border-white/20 rounded-xl">
                  ...
                  <a 
                    ...
                    className="inline-flex items-center gap-2 text-brand-accent hover:text-brand-accent/90 text-sm font-medium transition-colors"
                  >
                    <MapPin size={16} />
                    Ver en Google Maps
                  </a>
        ...
        <div className="p-4 bg-brand-surface border-t border-white/10">
           <button 
             ...
             form="checkout-form"
             disabled={loading}
             className="w-full bg-brand-accent hover:bg-brand-accent/90 disabled:opacity-50 disabled:cursor-not-allowed text-brand-dark font-bold py-4 rounded-2xl transition-transform active:scale-95 flex items-center justify-center gap-2"
           >
             {loading ? (
