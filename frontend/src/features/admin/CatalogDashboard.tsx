...
export const CatalogDashboard = () => {
  ...
  const fetchCatalog = async () => {
    try {
      ...
      // The API returns them ordered by 'order' asc
  ...
  // --- Category Handlers ---
  const handleOpenCatModal = (category: any = null) => {
    if (category) {
  ...
  const handleCatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        order: Number(catFormData.order)
      };

      if (editingCategory) {
        await api.patch(`/catalog/categories/${editingCategory.id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
  ...
  const moveCategory = async (id: number, currentOrder: number, direction: 'up' | 'down') => {
    const currentIndex = categories.findIndex(c => c.id === id);
    if (direction === 'up' && currentIndex > 0) {
      const other = categories[currentIndex - 1];
      await api.patch(`/catalog/categories/${id}`, { order: other.order }, { headers: { Authorization: `Bearer ${token}` } });
      await api.patch(`/catalog/categories/${other.id}`, { order: currentOrder }, { headers: { Authorization: `Bearer ${token}` } });
      fetchCatalog();
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight">Catálogo y Menú</h2>
          <p className="text-zinc-400">Gestioná los productos y categorías de tu menú.</p>
        </div>
        <button 
          onClick={() => activeTab === 'products' ? handleOpenModal() : handleOpenCatModal()}
      ...
      <div className="flex gap-4 mb-6 border-b border-zinc-800">
        <button 
          ...
          className={`pb-3 px-4 font-semibold text-sm transition-colors border-b-2 ${activeTab === 'products' ? 'border-amber-500 text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
        >
          Productos
        </button>
        <button 
          onClick={() => setActiveTab('categories')}
          className={`pb-3 px-4 font-semibold text-sm transition-colors border-b-2 ${activeTab === 'categories' ? 'border-amber-500 text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
        >
          Categorías
        </button>
      </div>
                  ...
                  <tr key={cat.id} className="transition-colors hover:bg-zinc-800/30">
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1 w-fit">
                        <button disabled={index === 0} onClick={() => moveCategory(cat.id, cat.order, 'up')} className={`p-1 rounded bg-zinc-800 ${index === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-700 hover:text-white transition-colors'}`}>
                          <ArrowUp size={14} />
                        </button>
                        <button disabled={index === categories.length - 1} onClick={() => moveCategory(cat.id, cat.order, 'down')} className={`p-1 rounded bg-zinc-800 ${index === categories.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-zinc-700 hover:text-white transition-colors'}`}>
                          <ArrowDown size={14} />
                        </button>
                      </div>
                    </td>
            ...
            <form onSubmit={handleCatSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 transition-all"
                  placeholder="Ej: Combos, Bebidas"
                />
              </div>

              <div>
                <label className="block text-zinc-400 text-sm font-medium mb-1">Descripción (Opcional)</label>
                <textarea 
                  rows={2}
                  value={catFormData.description}
                  onChange={(e) => setCatFormData({...catFormData, description: e.target.value})}
                  className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 transition-all resize-none"
                  placeholder="Ej: Nuestras mejores opciones"
                />
              </div>
